...
const formSchema = z.object({
  ...
  message: 'Stringmust be a valid hex code'
  })
});
...
export const ColorForm:React.FC<ColorFormProps> = ({
  ...
  const router = useRouter();
    ...
    const title = initialData ? 'Editer la couleur' : "Crée la couleur"
    const description = initialData ? 'Editer la couleur' : "Crée la nouvelle couleur"
    const toastMessage = initialData ? 'Couleur modifié' : "Couleur crée."
   ...
   const onDelete = async () => {
    try {
      ...
      toast.error("Voulez-vous supprimé cette couleur.")
    }finally{
      setLoading(false)
      setOpen(false)
    }
